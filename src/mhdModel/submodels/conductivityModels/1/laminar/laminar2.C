...
addToRunTimeSelectionTable(conductivityModel, constantSigma, conductivityModel);
...
:
    conductivityModel(rho, U, phi, thermophysicalModel, conductivityModelName)
{}


// * * * * * * * * * * * * * * * * * Selectors * * * * * * * * * * * * * * * //

autoPtr<constantSigma> constantSigma::New
(
    const volScalarField& rho,
    const volVectorField& U,
    const surfaceScalarField& phi,
...
{
    return autoPtr<constantSigma>
    (
        new laminar2(rho, U, phi, thermophysicalModel, conductivityModelName)
    );
}

...
{
    label nParcels = 0;
    label nMols = 0; // NEW VINCENT
    label noTotalCells = 0; // NEW VINCENT
    
    //1. measure number of molecules
    {
        const labelList& cells = mesh_.cellZones()[regionId_];
        noTotalCells = cells.size(); // NEW VINCENT
            ...
            forAll(molsInCell, mIC)
            {
                ...
                if(findIndex(typeIds_, typeId) != -1) 
                {
                    if(cloud_.axisymmetric())
                    {
                        ...
                        nParcels += RWF;
                        nMols += RWF*cloud_.nParticle(cell);
                    }
                    else
                    {
                        nParcels++;
                        nMols += cloud_.nParticle(cell);
                    }
                }
            }
        }
    ...
    /*Info << "nParcels (measured) = " << nParcels 
        ...
        << endl;*/
    Info << "nParcels (measured) = " << nMols
        << ", measured number density = " << nMols/volume_
        << ", target number density = " << density_
        << endl;    
    ...
    label nParcelsNew = label(density_*volume_/(nMols/noTotalCells)) - nParcels; // NEW VINCENT TODO to check kind of average for entire field. Make sense??
    bool insert = true;

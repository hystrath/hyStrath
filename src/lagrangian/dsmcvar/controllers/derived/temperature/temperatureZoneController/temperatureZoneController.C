    ...
    if(timeVel_.samplingTime())
    {
        ...
        forAll(cells, c)
        {
            ...
            forAll(molsInCell, mIC)
            {
                ...
                {
                    //scalar nParticleFactor = cloud_.nParticle();
                    ...
                    const scalar mass = cloud_.constProps(p->typeId()).mass()*cloud_.nParticle(cellI)*nParticleFactor;
        ...
        forAll(cells, c)
        {
            ...
            forAll(molsInCell, mIC)
            {
                ...
                if(findIndex(typeIds_, p->typeId()) != -1)
                {
                    ...
                    scalar mass = cloud_.constProps(p->typeId()).mass()*cloud_.nParticle(cell)*nParticleFactor;
                    
                    mcc_ += mass*mag(p->U())*mag(p->U());
                    m_ += mass;
                    //nParcels_ += nParticle;
                    nParcels_ += cloud_.nParticle(cell)*nParticleFactor;
                }
            }
        }
    }

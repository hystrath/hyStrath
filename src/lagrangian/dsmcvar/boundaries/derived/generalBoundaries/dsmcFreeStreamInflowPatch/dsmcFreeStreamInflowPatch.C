...
void dsmcFreeStreamInflowPatch::controlParcelsBeforeMove()
{
    ...
    //const scalar deltaT = mesh_.time().deltaTValue(); // DELETED VINCENT
    ...
    forAll(accumulatedParcelsToInsert_, i)
    {
        ...
        forAll(accumulatedParcelsToInsert_[i], f)
        {
            ...
            // From Bird eqn 4.22
            const label cellI = mesh_.faceOwner()[faceI]; // NEW VINCENT
            ...
            if(cloud_.axisymmetric())
            {
                ...
                /(2.0*sqrtPi*cloud_.nParticle(cellI)*RWF);
            }
            else
            {
                 accumulatedParcelsToInsert_[i][f] += 
                ...
                /(2.0*sqrtPi*cloud_.nParticle(cellI));
            }
        }
    }
    ...
    forAll(faces_, f)
    {
     ...
     if (Pstream::parRun()) // RE-INSERTED VINCENT
    {

...
{            
    ...
    forAll(cells_, c)
    {
        ...
        forAll(parcelsInCell, pIC)
        {
            ...
            const scalar& mass = constProp.mass()*cloud_.nParticle(c);
                        
            totalMass_ += mass;
            totalMols_ += 1.0;
            totalMolsGas_ += cloud_.nParticle(c);
            mcc_ += mass*mag(p->U())*mag(p->U());
            incidentMomentum_ += cloud_.nParticle(c)*cloud_.constProps(p->typeId()).mass()*p->U();
            incidentMass_ += cloud_.nParticle(c)*cloud_.constProps(p->typeId()).mass();
...
//             vector tw2 = nw^tw1;
    ...
    scalar mols = mols_;
    scalar totalMols = totalMolsGas_;
    scalar cv = cv_;
    scalar mcc = mcc_;
    ...
    if(mols > VSMALL)
    {
        ...
        do
        {
            ...
            scalar averageMass = totalMass/totalMols;

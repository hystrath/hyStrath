...
void dsmcVolFields::writeOut()
{
    if (time_.time().outputTime())
    {
        ...
        /*wordList fieldsNameToWrite (9, word::null);  
        
        fieldsNameToWrite[0] = "rhoNMean";
        fieldsNameToWrite[1] = "rhoNMean"; 
        fieldsNameToWrite[2] = "rhoMMean";
        fieldsNameToWrite[3] = "linearKEMean";
        fieldsNameToWrite[4] = "momentumMean";
        fieldsNameToWrite[5] = "rotationalEMean";
        fieldsNameToWrite[6] = "rotationalDofMean";
        fieldsNameToWrite[7] = "vibrationalETotal";
        fieldsNameToWrite[8] = "nParcels";
        fieldsNameToWrite[9] = "rhoNMeanInt";
        
        forAll(fieldsNameToWrite, i)
        {
            volScalarField fieldToWrite
            (
                IOobject
                (
                    fieldsNameToWrite[i]+"_"+fieldName_,
                    time_.time().timeName(),
                    //"uniform",
                    mesh_,
                    IOobject::NO_READ,
            ...
            fieldToWrite.internalField() = rhoNMean_;
...
{  
    ...
    //const scalar& nParticle = cloud_.nParticle(); // DELETED VINCENT
...
//                     scalar radius = sqrt((p.position().y()*p.position().y()) + (p.position().z()*p.position().z()));
                    ...
                    rhoNMeanXnParticle_[cell] += (RWF*cloud_.nParticle(cell));
                    rhoMMeanXnParticle_[cell] += (mass*RWF*cloud_.nParticle(cell));
                }
                else
                {
                    rhoNMeanXnParticle_[cell] += cloud_.nParticle(cell);
                    rhoMMeanXnParticle_[cell] += (mass*cloud_.nParticle(cell));
                }
            }
        }
    }
...
//                     Info << "RWF = " << RWF << endl;
                    ...
                    nParcelsXnParticle_[iD][cell] += (RWF*cloud_.nParticle(cell));
                    rhoNMeanXnParticle_[cell] += (RWF*cloud_.nParticle(cell));
                    rhoMMeanXnParticle_[cell] += (mass*RWF*cloud_.nParticle(cell));
                    momentumMeanXnParticle_[cell] += (mass*(p.U())*RWF*cloud_.nParticle(cell));
                    linearKEMeanXnParticle_[cell] += (mass*(p.U() & p.U())*RWF*cloud_.nParticle(cell));
                }
                else
                {
                    nParcelsXnParticle_[iD][cell] += cloud_.nParticle(cell);
                    rhoNMeanXnParticle_[cell] += cloud_.nParticle(cell);
                    rhoMMeanXnParticle_[cell] += (mass*cloud_.nParticle(cell));
                    momentumMeanXnParticle_[cell] += (mass*(p.U())*cloud_.nParticle(cell));
                    linearKEMeanXnParticle_[cell] += (mass*(p.U() & p.U())*cloud_.nParticle(cell));
                }
                    ...
                    if(!isA<emptyPolyPatch>(patch))
                    {
                        if(!isA<cyclicPolyPatch>(patch))
                        {
                            forAll(boundaryCells_[j], k)
                            {
                                ...
                                cellMfpRatio_.boundaryFieldRef()[j][k] = cellMfpRatio_[boundaryCells_[j][k]];
...
//             rhoM_.correctBoundaryConditions();
            ...
            forAll(rhoNBF_, j)
            {
                ...
                {                               
                    ...
                    {                        
                        const label cellI = patch.faceCells()[k];
                        
                        rhoN_.boundaryFieldRef()[j][k] = rhoNBF_[j][k]*cloud_.nParticle(cellI)/nAvTimeSteps;
                        rhoM_.boundaryFieldRef()[j][k] = rhoMBF_[j][k]*cloud_.nParticle(cellI)/nAvTimeSteps;
                        
                        if(rhoM_.boundaryField()[j][k] > VSMALL)
                        {
                            UMean_.boundaryFieldRef()[j][k] = momentumBF_[j][k]*cloud_.nParticle(cellI)/(rhoM_.boundaryField()[j][k]*nAvTimeSteps);
                        }
                        else
                        {
                        ...
                        scalar rhoMMean = rhoMBF_[j][k]*cloud_.nParticle(cellI)/nAvTimeSteps;
                        scalar linearKEMean = linearKEBF_[j][k]*cloud_.nParticle(cellI)/nAvTimeSteps;
                        scalar rhoNMean = rhoNBF_[j][k]*cloud_.nParticle(cellI)/nAvTimeSteps;

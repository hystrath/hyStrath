...
defineTypeNameAndDebug(dsmcHybridFields, 0);

addToRunTimeSelectionTable(dsmcField, dsmcHybridFields, dictionary);
...
dsmcHybridFields::dsmcHybridFields
(
    Time& t,
    const polyMesh& mesh,
    dsmcCloud& cloud,
    ...
    vibrationalTs_()
{
    ...
    forAll(moleculesReduced, i)
    {
        ...
        if(typeId == -1)
        {
            FatalErrorIn("dsmcHybridFields::dsmcHybridFields()")
    ...
    forAll(mcr_, i)
    {
        ...
        (
            ...
            (
                IOobject
                (
                    "vibrationalT_" + moleculesReduced[i],
                    mesh_.time().timeName(),
                    mesh_,
                    IOobject::NO_READ,
                    IOobject::AUTO_WRITE
...
void dsmcHybridFields::writeOut()
{
...
//- initial conditions
void dsmcHybridFields::createField()
{
    Info << "Initialising dsmcHybridFields field" << endl;
}


void dsmcHybridFields::calculateField()
{  
    nTimeSteps_ += 1.0;
    ...
    if(time_.time().outputTime())
    {
        ...
        forAll(rhoNMean_, cell)
        {
            if(rhoNMean_[cell] > VSMALL)
            {
                ...
                {
                    label iD = findIndex(typeIds_, i);
                    rhoNs_[iD][cell] = (nParcels_[iD][cell] * cloud_.nParticle())
                        / (nAvTimeSteps * V);
                }
            ...
            forAll(vibrationalETotal_, iD)
            {
                ...
                {        
                    ...
                    vibrationalTs_[iD][cell] = vibT_[iD][cell];
                ...
                if(!isA<emptyPolyPatch>(patch))
                {
                    if(!isA<cyclicPolyPatch>(patch))
                    {
                        ...
                        {       
                            ...
                            forAll(vibrationalTs_, i)
                            {
                                rhoNs_[i].boundaryField()[j][k]
                                    = rhoNs_[i][boundaryCells_[j][k]];
                                vibrationalTs_[i].boundaryField()[j][k]
                                    = vibrationalTs_[i][boundaryCells_[j][k]];
                            }
                        }
                    }
                }
...
//             }
        ...
        forAll(rhoNBF_, j)
        {
            ...
            if(isA<wallPolyPatch>(patch))
            {
                ...
                {
                    rhoN_.boundaryFieldRef()[j][k] = rhoNBF_[j][k]
                        * cloud_.nParticle() / nAvTimeSteps;
                    rhoM_.boundaryFieldRef()[j][k] = rhoMBF_[j][k]
                        * cloud_.nParticle() / nAvTimeSteps;
                    forAll(rhoNs_, i)
                    {
                        label iD = findIndex(typeIds_, i);
                        rhoNs_[iD].boundaryField()[j][k]
                            = speciesRhoNBF_[i][j][k] * cloud_.nParticle()
                            / nAvTimeSteps;
                    }
                }
            }
                ...
                forAll(rhoN_.boundaryField()[j], k)
                {
                    ...
                    forAll(vibrationalEBF_, i)
                    {
                        ...
                        {        
                            ...
                            vibrationalTs_[i].boundaryFieldRef()[j][k] = vibTBF_[i][j][k];
...
//- write field
void dsmcHybridFields::writeField()
{}

void dsmcHybridFields::getDSMCfields
(
    volScalarField& transrotationalT,
    volScalarField& rho,
    volScalarField& diffT,
    volVectorField& U,
    PtrList<volScalarField>& vibrationalTs,
...
)
{
    ...
    forAll(typeIds_, iD)
    {
        dimensionedScalar mass("mass", dimMass, cloud_.constProps(iD).mass());
        dimensionedScalar rhoNDim("rhoNDim", dimless / dimVolume, 1.0);
        scalar rotDofId = cloud_.constProps(iD).rotationalDegreesOfFreedom();
        vibrationalTs[iD] = vibrationalTs_[iD];
        volScalarField rhoIByRho = 0.0 * rhoNs_[iD] / rhoNDim;
        volScalarField rotDofLocal = rotDofId * rhoNs_[iD] / rhoNDim;
        forAll(mesh_.cells(), cellI)
        {
            if(rhoN_[cellI] > VSMALL)
            {
                rotDofLocal[cellI] /= rhoN_[cellI];
                rhoIByRho[cellI] = mass.value() * rhoNs_[iD][cellI]
                    / rhoM_[cellI];
            }
        }
        forAll(rhoN_.boundaryField(), boundaryI)
        {
            forAll(rhoN_.boundaryField()[boundaryI], faceI)
            {
                if(rhoN_.boundaryField()[boundaryI][faceI] > VSMALL)
                {
                    rotDofLocal.boundaryField()[boundaryI][faceI]
                        /= rhoN_.boundaryField()[boundaryI][faceI];
                    rhoIByRho.boundaryField()[boundaryI][faceI]
                        = mass.value()
                        * rhoNs_[iD].boundaryField()[boundaryI][faceI]
                        / rhoM_.boundaryField()[boundaryI][faceI];
                }
            }
        }
        vibrationalTs[iD] = vibrationalTs_[iD];
    ...
    transrotationalT = (3.0 * translationalT_ + rotDOF * rotationalT_)
        / (3.0 + rotDOF);
    ...
    forAll(mesh_.cells(), cellI)
    {
        if(rotationalT_[cellI] > VSMALL)
        {
            diffT[cellI] = 5.0 * abs(translationalT_[cellI]
                / rotationalT_[cellI] - 1.0);
        }
    }

    forAll(rotationalT_.boundaryField(), boundaryI)
    {
        forAll(rotationalT_.boundaryField()[boundaryI], faceI)
        {
            if(rotationalT_.boundaryField()[boundaryI][faceI] > VSMALL)
            {
                diffT.boundaryField()[boundaryI][faceI] = 5.0 * abs(
                    translationalT_.boundaryField()[boundaryI][faceI]
                    / rotationalT_.boundaryField()[boundaryI][faceI] - 1.0);
            }
        }
    }
}
...
void dsmcHybridFields::updateProperties(const dictionary& newDict)
{

...
addToRunTimeSelectionTable(dsmcField, dsmcVolFieldsFromFields, dictionary);
    ...
    measureErrors_(false)
{
    ...
    forAll(moleculesReduced, i)
    {
        ...
        if(typeId == -1)
        {
            FatalErrorIn("dsmcVolFieldsFromFields::dsmcVolFieldsFromFields()")
    ...
    {
        vibrationalETotal_[i].setSize(mesh_.nCells(), 0.0);
    }
    ...
    forAll(boundaryCells_, p)
    {
        ...
        boundaryCells_[p].setSize(patch.size(), 0);
    ...
    if (nTimeSteps_ > SMALL)
    {
        ...
        const word cloudName = propsDict_.lookupOrDefault<word>("cloudName", "dsmc");

        IOdictionary dsmcProperties_
        (
            IOobject
            (
        ...
        scalarField V_ = mesh_.cellVolumes();

        forAll(typeIds_, iD)
        {
            const dictionary moleculeProperties =
                dsmcProperties_.subDict(
                "moleculeProperties").subDict(moleculesReduced[iD]);
            const scalar thetaV = moleculeProperties.lookupOrDefault<scalar>(
                "characteristicVibrationalTemperature", 3371.0);
            const scalar rotDof = moleculeProperties.lookupOrDefault<scalar>(
                "rotationalDegreesOfFreedom", 0.0);
            const scalar vibModes = moleculeProperties.lookupOrDefault<scalar>(
                "vibrationalModes", 0.0);

            volScalarField rhoNSpecies_
            (
                IOobject
                (
                    "rhoN_" + moleculesReduced[iD],
                    mesh_.time().timeName(),
                    mesh_,
                    IOobject::MUST_READ,
                    IOobject::NO_WRITE
            ...
            volScalarField vibrationalTSpecies_
            (
                IOobject
                (
                    "vibrationalT_" + moleculesReduced[iD],
                    mesh_.time().timeName(),
                    mesh_,
                    IOobject::MUST_READ,
                    IOobject::NO_WRITE
            ...
            {
                scalar nTVBynEq = nTBynEq * V_[cell];
                nParcels_[iD][cell] = rhoNSpecies_[cell] * nTVBynEq;
                if (rotDof > SMALL or vibModes > SMALL)
                {
                    rhoNMeanInt_[cell] += rhoNSpecies_[cell] * nTVBynEq;
                    rotationalDofMean_ += rotDof * rhoNSpecies_[cell]
                        * nTVBynEq;
                    if (vibrationalTSpecies_[cell] > VSMALL)
                    {
                        vibrationalETotal_[iD][cell] = nParcels_[iD][cell] * k
                            * thetaV / (exp(thetaV
                            / vibrationalTSpecies_[cell]) - 1.0);
                    }
                    eVib_[cell] += vibrationalETotal_[iD][cell];
                }
            }

            if (rotDof > SMALL or vibModes > SMALL)
            {
                forAll(rhoNSpecies_.boundaryField(), j)
                {
                    forAll(rhoNSpecies_.boundaryField()[j], l)
                    {
                        rotationalDofBF_[j][l] += rotDof
                            * rhoNSpecies_.boundaryField()[j][l] * nTBynEq
                            * time_.time().deltaT().value();
                        speciesRhoNBF_[iD][j][l] =
                            rhoNSpecies_.boundaryField()[j][l] * nTBynEq
                            * time_.time().deltaT().value();
                        speciesRhoNIntBF_[j][l] +=
                            rhoNSpecies_.boundaryField()[j][l] * nTBynEq
                            * time_.time().deltaT().value();
                        if (vibrationalTSpecies_.boundaryField()[j][l] > VSMALL)
                        {
                            vibrationalEBF_[iD][j][l] =
                                speciesRhoNBF_[iD][j][l] * k * thetaV
                                / (exp(thetaV
                                / vibrationalTSpecies_.boundaryField()[j][l])
                                - 1.0);
                        }
                    }
                }
        ...
        forAll(mesh_.cells(), cell)
        {
            scalar nTVBynEq = nTimeSteps_ * V_[cell] / cloud_.nParticle();
            rhoNMean_[cell] = rhoN_[cell] * nTVBynEq;
            rhoMMean_[cell] = rhoM_[cell] * nTVBynEq;
            momentumMean_[cell] = rhoM_[cell] * UMean_[cell] * nTVBynEq;
            linearKEMean_[cell] = (3.0 * k * rhoN_[cell]
                * translationalT_[cell] + rhoM_[cell]
                * (UMean_[cell] & UMean_[cell])) * nTVBynEq;

            rotationalEMean_ = 0.5 * rotationalT_ * k * rotationalDofMean_;

            muu_[cell] = (pressureTensor_[cell].xx() + rhoM_[cell]
                * UMean_[cell].x() * UMean_[cell].x()) * nTVBynEq;
            muv_[cell] = (pressureTensor_[cell].xy() + rhoM_[cell]
                * UMean_[cell].x() * UMean_[cell].y()) * nTVBynEq;
            muw_[cell] = (pressureTensor_[cell].xz() + rhoM_[cell]
                * UMean_[cell].x() * UMean_[cell].z()) * nTVBynEq;
            mvv_[cell] = (pressureTensor_[cell].yy() + rhoM_[cell]
                * UMean_[cell].y() * UMean_[cell].y()) * nTVBynEq;
            mvw_[cell] = (pressureTensor_[cell].yz() + rhoM_[cell]
                * UMean_[cell].y() * UMean_[cell].z()) * nTVBynEq;
            mww_[cell] = (pressureTensor_[cell].zz() + rhoM_[cell]
                * UMean_[cell].z() * UMean_[cell].z()) * nTVBynEq;

            mcc_[cell] = linearKEMean_[cell];
            mccu_[cell] = mcc_[cell] * UMean_[cell].x() + 2.0 *
                (
                    pressureTensor_[cell].xx() * UMean_[cell].x()
                    + pressureTensor_[cell].xy() * UMean_[cell].y()
                    + pressureTensor_[cell].xz() * UMean_[cell].z()
                    + heatFluxTraVector_[cell].x()
                ) * nTVBynEq;
            mccv_[cell] = mcc_[cell] * UMean_[cell].y() + 2.0 *
                (
                    pressureTensor_[cell].yx() * UMean_[cell].x()
                    + pressureTensor_[cell].yy() * UMean_[cell].y()
                    + pressureTensor_[cell].yz() * UMean_[cell].z()
                    + heatFluxTraVector_[cell].y()
                ) * nTVBynEq;
            mccw_[cell] = mcc_[cell] * UMean_[cell].z() + 2.0 *
                (
                    pressureTensor_[cell].zx() * UMean_[cell].x()
                    + pressureTensor_[cell].zy() * UMean_[cell].y()
                    + pressureTensor_[cell].zz() * UMean_[cell].z()
                    + heatFluxTraVector_[cell].z()
                ) * nTVBynEq;

            eRot_[cell] = rotationalEMean_[cell];
            eRotU_[cell] = heatFluxRotVector_[cell].x() * nTVBynEq
                + eRot_[cell] * UMean_[cell].x();
            eRotV_[cell] = heatFluxRotVector_[cell].y() * nTVBynEq
                + eRot_[cell] * UMean_[cell].y();
            eRotW_[cell] = heatFluxRotVector_[cell].z() * nTVBynEq
                + eRot_[cell] * UMean_[cell].z();
            ...
            {            
                nClassI_[cell] = classIDistribution_[cell] * rhoNMean_[cell];
                nClassII_[cell] = classIIDistribution_[cell] * rhoNMean_[cell];
                nClassIII_[cell] = classIIIDistribution_[cell] * rhoNMean_[cell];
            }
        }

        forAll(rhoN_.boundaryField(), j)
        {
            forAll(rhoN_.boundaryField()[j], l)
            {
                rhoNBF_[j][l] = rhoN_.boundaryField()[j][l] * nTBynEq
                    * time_.time().deltaT().value();
                rhoMBF_[j][l] = rhoM_.boundaryField()[j][l] * nTBynEq
                    * time_.time().deltaT().value();
                linearKEBF_[j][l] = 0.5 * (3.0 * k * rhoNBF_[j][l]
                    * translationalT_.boundaryField()[j][l] + rhoMBF_[j][l]
                    * (UMean_.boundaryField()[j][l] & UMean_.boundaryField()[j][l]));
                momentumBF_[j][l] = rhoMBF_[j][l] * UMean_.boundaryField()[j][l];

                rotationalEBF_[j][l] = 0.5 * k * rotationalDofBF_[j][l]
                    * rotationalT_.boundaryField()[j][l];

                qBF_[j][l] = q_.boundaryField()[j][l] * nTimeSteps_;
                fDBF_[j][l]= fD_.boundaryField()[j][l] * nTimeSteps_;
            }
        }
    }
...
void dsmcVolFieldsFromFields::readIn()
{
    IOdictionary dict
    (
        IOobject
...
void dsmcVolFieldsFromFields::writeOut()
{
...
//- initial conditions
void dsmcVolFieldsFromFields::createField()
{
    Info << "Initialising dsmcVolFieldsFromFields field" << endl;
}
...
/*    volScalarField dummyVal
    ...
    forAllConstIter(dsmcCloud, cloud_, iter)
    {
        ...
        if(iD != -1)
        {
            ...
            const scalar& vibrationalDof
                = cloud_.constProps(p.typeId()).vibrationalDegreesOfFreedom();
            ...
            //vibrationalETotal_[iD][cell] += p.EVib(); TODO
            ...
            /*eVibU_[cell] += p.EVib() * (p.U().x()); TODO
            eVibV_[cell] += p.EVib() * (p.U().y());
            eVibW_[cell] += p.EVib() * (p.U().z());*/
            eRot_[cell] += p.ERot();
            //eVib_[cell] += p.EVib(); TODO
            
            if(rotationalDof > VSMALL or vibrationalDof > VSMALL)
            {
    ...
    forAll(cloud_.typeIdList(), i)
    {
        label iD = findIndex(typeIds_, i);
        if(iD != -1)
        { 
            forAll(speciesRhoNBF_[iD], j)
            {
                forAll(speciesRhoNBF_[iD][j], k)
                {
                    speciesRhoNBF_[iD][j][k] += cloud_.boundaryFluxMeasurements().rhoNBF()[i][j][k];
                    speciesRhoNIntBF_[j][k] += cloud_.boundaryFluxMeasurements().rhoNBF()[i][j][k];
        ...
        forAll(rhoNMean_, cell)
        {
            ...
            if(rhoNMean_[cell] > VSMALL)
            {
                ...
                heatFluxRotVector_[cell].x() = rhoN_[cell] * (eRotU_[cell]
                    - eRot_[cell] * UMean_[cell].x()) / rhoNMean_[cell];
                ...
                heatFluxRotVector_[cell].y() = rhoN_[cell] * (eRotV_[cell]
                    - eRot_[cell] * UMean_[cell].y()) / rhoNMean_[cell];
                ...
                heatFluxRotVector_[cell].z() = rhoN_[cell] * (eRotW_[cell]
                    - eRot_[cell] * UMean_[cell].z()) / rhoNMean_[cell];
        ...
        forAll(rhoNBF_, j)
        {
            ...
            if(isA<wallPolyPatch>(patch))
            {
                forAll(rhoN_.boundaryField()[j], k)
                {
                    ...
                    rhoN_.boundaryFieldRef()[j][k] /= time_.time().deltaT().value();
                    rhoM_.boundaryFieldRef()[j][k] /= time_.time().deltaT().value();
                    ...
                    forAll(mesh.boundaryMesh(), i)
                    {
                        ...
                        const vectorField fC_DSMC = patch.faceCentres();
                        if (isA<wallPolyPatch>(patch))
                        {
                            ...
                            forAll(t1, f)
                            {
                                t1[f] = fC_DSMC[f] - mesh_.points()[mesh_.faces()[faces[f]][0]];
                            }

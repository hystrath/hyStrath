...
defineTypeNameAndDebug(dsmcSubrelax, 0);

addToRunTimeSelectionTable(dsmcField, dsmcSubrelax, dictionary);
    ...
    stopSamplingReset_(false)
{
    ...
    forAll(moleculesReduced, i)
    {
        ...
        if(typeId == -1)
        {
            FatalErrorIn("dsmcSubrelax::dsmcSubrelax()")
...
//- initial conditions
void dsmcSubrelax::createField()
{
    Info << "Initialising dsmcSubrelax field" << endl;
}


void dsmcSubrelax::calculateField()
{  
    nTimeSteps_++;
    ...
    rhoNMean_ *= factor1;
    rhoMMean_ *= factor1;
    linearKEMean_ *= factor1;
    momentumMean_ *= factor1;
    rotationalEMean_ *= factor1;
    rotationalDofMean_ *= factor1;
    muu_ *= factor1;
    muv_ *= factor1;
    muw_ *= factor1;
    mvv_ *= factor1;
    ...
    forAll(typeIds_, iD)
    {
        vibrationalETotal_[iD] *= factor1;
        nParcels_[iD] *= factor1;
    }
    rhoNMeanInt_ = rhoNMeanInt_ *  factor1;
    ...
    forAllConstIter(dsmcCloud, cloud_, iter)
    {
        ...
        if(iD != -1)
        {
            ...
            rhoNMean_[cell] += 1.0 * relaxFactor;
            rhoMMean_[cell] += mass * relaxFactor;
            linearKEMean_[cell] += mass * (p.U() & p.U()) * relaxFactor;
            momentumMean_[cell] += mass * p.U() * relaxFactor;
            rotationalEMean_[cell] += p.ERot() * relaxFactor;
            rotationalDofMean_[cell] += rotationalDof * relaxFactor; 
            vibrationalETotal_[iD][cell] += p.EVib() * relaxFactor;
            nParcels_[iD][cell] += 1.0 * relaxFactor;
            
            muu_[cell] += mass * sqr(p.U().x()) * relaxFactor;
            muv_[cell] += mass * ( (p.U().x()) * (p.U().y()) ) * relaxFactor;
            muw_[cell] += mass * ( (p.U().x()) * (p.U().z()) ) * relaxFactor;
            mvv_[cell] += mass * sqr(p.U().y()) * relaxFactor;
            mvw_[cell] += mass * ( (p.U().y()) * (p.U().z()) ) * relaxFactor;
            mww_[cell] += mass * sqr(p.U().z()) * relaxFactor;
            
            mcc_[cell] += mass * mag(p.U()) * mag(p.U()) * relaxFactor;
            ...
            eu_[cell] += ( p.ERot() + p.EVib() ) * (p.U().x()) * relaxFactor;
            ev_[cell] += ( p.ERot() + p.EVib() ) * (p.U().y()) * relaxFactor;
            ew_[cell] += ( p.ERot() + p.EVib() ) * (p.U().z()) * relaxFactor;
            e_[cell] += ( p.ERot() + p.EVib() ) * relaxFactor;

            if(rotationalDof > VSMALL)
            {
                rhoNMeanInt_[cell] += 1.0 * relaxFactor;
            }
            ...
            if(measureClassifications_)
            {
                ...
                if(classification == 0)
                {
                    nClassI_[cell] += 1.0 * relaxFactor;
                }
                
                if(classification == 1)
                {
                    nClassII_[cell] += 1.0 * relaxFactor;
                }
                
                if(classification == 2)
                {
                    nClassIII_[cell] += 1.0 * relaxFactor;
                }
            }
        }
    }
    ...
    Info << "valueLocal " << dummyScalar << endl;
            ...
            {                
                forAll(cloud_.boundaryFluxMeasurements().rhoNBF()[i][j], k)
                {
                    if(cloud_.boundaryFluxMeasurements().rhoNBF()[i][j][k] > VSMALL)
                    {
                        rhoNBF_[j][k] *= factor1;
                        rhoMBF_[j][k] *= factor1;
                        linearKEBF_[j][k] *= factor1;
                        momentumBF_[j][k] *= factor1;
                        rotationalEBF_[j][k] *= factor1;
                        rotationalDofBF_[j][k] *= factor1;
                        qBF_[j][k] *= factor1;
                        fDBF_[j][k] *= factor1;
                    }
                }
        ...
        { 
            ...
            {                
                forAll(speciesRhoNBF_[i][j], k)
                {
                    ...
                    {
                        speciesRhoNBF_[i][j][k] *= factor1;
                        vibrationalEBF_[i][j][k] *= factor1;
                    }
                }
            }
        }
    ...
    forAll(speciesRhoNIntBF_, j)
    {
        forAll(speciesRhoNIntBF_[j], k)
        {
            ...
            {
                speciesRhoNIntBF_[j][k] = speciesRhoNIntBF_[j][k] * factor1;
            }
        }
    }
...
void dsmcSubrelax::setInitialFields
(
...
//- write field
void dsmcSubrelax::writeField()
{}

void dsmcSubrelax::updateProperties(const dictionary& newDict)
{

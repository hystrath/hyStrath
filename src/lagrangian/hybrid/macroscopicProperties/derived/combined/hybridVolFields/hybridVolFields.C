...
defineTypeNameAndDebug(hybridVolFields, 0);

addToRunTimeSelectionTable(dsmcField, hybridVolFields, dictionary);
...
hybridVolFields::hybridVolFields
(
    Time& t,
    const polyMesh& mesh,
    dsmcCloud& cloud,
    ...
    stopSamplingReset(false)
{
    ...
    forAll(moleculesReduced, i)
    {
        ...
        if(typeId == -1)
        {
            FatalErrorIn("hybridVolFields::hybridVolFields()")
...
//- initial conditions
void hybridVolFields::createField()
{
    Info << "Initialising hybridVolFields field" << endl;
}


void hybridVolFields::calculateField()
{  
    nTimeSteps_ += 1.0;
...
//         tau_.correctBoundaryConditions();
        ...
        //- reset
        if(time_.resetFieldsAtOutput() and not stopSamplingReset)
        {
            nTimeSteps_ = 0.0;
...
void hybridVolFields::setReset
(
    bool& stopReset
)
{
    stopSamplingReset = stopReset;
}


//- write field
void hybridVolFields::writeField()
{}

void hybridVolFields::updateProperties(const dictionary& newDict)
{

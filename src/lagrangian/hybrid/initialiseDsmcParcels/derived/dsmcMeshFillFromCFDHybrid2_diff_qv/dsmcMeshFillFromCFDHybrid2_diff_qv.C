...
#include "dsmcMeshFillFromCFDHybrid2_diff_qv.H"
...
dsmcMeshFillFromCFDHybrid2_diff_qv::dsmcMeshFillFromCFDHybrid2_diff_qv
(
    dsmcCloud& cloud,
...
void dsmcMeshFillFromCFDHybrid2_diff_qv::setInitialConfiguration()
{
    ...
    bool diff_ = dsmcInitialiseDict_.lookupOrDefault<bool>("diff",false);
    bool qv_ = dsmcInitialiseDict_.lookupOrDefault<bool>("qv",false);
    bool qvpD_ = dsmcInitialiseDict_.lookupOrDefault<bool>("qvPartialD",false);
...
//------------------------------------------------------------------------------
    ...
    forAll(molecules, moleculeI)
    {
        ...
        nDTotInitial += nDInitial[moleculeI];
    }
    ...
    forAll(molecules, moleculeI)
    {
        ...
        forAll(mesh_.cells(), cellI)
        {
            if(nDInitial[moleculeI][cellI] > VSMALL)
            {
                ...
                qt[moleculeI][cellI] = -(15.0 / (15.0 + 2.0  * rotDoF))
                    * (kappatrInitial[moleculeI][cellI]
//                    / pInitial[moleculeI][cellI])
                ...
                if(rotDoF > SMALL)
                {
                    qr[moleculeI][cellI] = -(2.0 * rotDoF / (15.0 + 2.0 * rotDoF))
...
//                        / pInitial[moleculeI][cellI])
                ...
                if(vibDoF > SMALL)
                {
                    if(qv_)
                    {
                        qv[moleculeI][cellI] = -muInitial[moleculeI][cellI]
                        * dummyGradTv[cellI] * twoBeta
                        / TvInitial[moleculeI][cellI];
                        if(qvpD_)
                        {
                            qv[moleculeI][cellI] /= (mass.value()
                                * nDInitial[moleculeI][cellI]);
                        }
                        else
                        {
//                            qv[moleculeI][cellI] /= rhoTotInitial[cellI];
                            qv[moleculeI][cellI] /= (mass.value()
                                * nDTotInitial[cellI]);
                        }
                    }
    ...
    //                        / pInitial[moleculeI][cellI])
                            ...
                            * TvInitial[moleculeI][cellI] / pow(thetaV[0], 2))
                ...
                if(diff_)
                {
                    D[moleculeI][cellI] /= rhoTotInitial[cellI];
                }
                else
                {
                    D[moleculeI][cellI] /= (mass.value()
                        * nDInitial[moleculeI][cellI]);
                }

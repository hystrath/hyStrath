...
(
    const label speciei, 
    const volScalarField::GeometricBoundaryField& tbf1T,
...
Foam::wordList Foam::basic2MultiComponentMixture::he2BoundaryBaseTypes(const volScalarField::GeometricBoundaryField& tbf)
{
    /*const volScalarField::GeometricBoundaryField& tbff =
    ...
    zetaElec_(species_.size())
{
    ...
    forAll(species_, i)
    {
        ...
        if (YHeader.headerOk() && TtHeader.headerOk())
        {
            headersOK = true;
        ...
        else if (XHeader.headerOk() && TtHeader.headerOk())
        {
            headersOK = true;
        ...
        {
            if (not YHeader.headerOk() and not XHeader.headerOk())
            {

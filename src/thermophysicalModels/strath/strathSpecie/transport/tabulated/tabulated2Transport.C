...
template<class Thermo>
Foam::tabulated2Transport<Thermo>::tabulated2Transport(const dictionary& dict)
:
    Thermo(dict)
{
    word muFile_ = dict.subDict("transport").lookup("muFile");
    word kappaFile_ = dict.subDict("transport").lookup("kappaFile");
    mu_ = interpolation2DTable<scalar>("constant/tabulatedData/" + muFile_);
    //mu_.outOfBounds(interpolation2DTable<scalar>::EXTRAPOLATE);
    kappa_ = interpolation2DTable<scalar>("constant/tabulatedData/" + kappaFile_);
    //kappa_.outOfBounds(interpolation2DTable<scalar>::EXTRAPOLATE);
}
...
template<class Thermo>
void Foam::tabulated2Transport<Thermo>::write(Ostream& os) const
{
...
template<class Thermo>
Foam::Ostream& Foam::operator<<(Ostream& os, const tabulated2Transport<Thermo>& tt)
{
    operator<<(os, static_cast<const Thermo&>(tt));

    os.check("Ostream& operator<<(Ostream&, const tabulated2Transport&)");

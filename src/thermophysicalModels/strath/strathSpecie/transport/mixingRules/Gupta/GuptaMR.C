...
void Foam::GuptaMR<ThermoType>::correct()
{
    ...
    scalarField& muCells = tmpMu.internalField();
    scalarField& kappatCells = tmpKappat.internalField();
    scalarField& kapparCells = tmpKappar.internalField();
    scalarField& kappaveCells = tmpKappave.internalField();
    scalarField& alphaveCells = tmpAlphave.internalField();
    ...
    forAll(species(), speciei)
    {
        ...
        { 
            ...
            scalarField& den1Cells = den1.internalField();
            scalarField& denkt1Cells = denkt1.internalField();
            scalarField& denkt2Cells = denkt2.internalField();
            scalarField& denkr1Cells = denkr1.internalField();
            scalarField& denkr2Cells = denkr2.internalField();
            ...
            forAll(species(), speciej)
            {
                ...
                if(speciesThermo_[speciej].particleType() > 0)  // heavy-particle - heavy-particle collision
                {
                    ...
                    if (speciesThermo_[speciei].particleCharge() == 0 or speciesThermo_[speciej].particleCharge() == 0) // collision involving at least one neutral
                    {
                        ...
                        forAll(pDj.boundaryField(), patchi)
                        {
                            ...
                            fvPatchScalarField& pden1 = den1.boundaryField()[patchi];
                            fvPatchScalarField& pdenkt1 = denkt1.boundaryField()[patchi];
                            fvPatchScalarField& pdenkr1 = denkr1.boundaryField()[patchi];
                ...
                else // the colliding partner of the heavy-particle is an electron
                {
                    ...
                    forAll(pDe.boundaryField(), patchi)
                    {
                        ...
                        const fvPatchScalarField& pnDe = nDe.boundaryField()[patchi];
                        fvPatchScalarField& pdenkt2 = denkt2.boundaryField()[patchi];
                        fvPatchScalarField& pdenkr2 = denkr2.boundaryField()[patchi];
            ...
            {     
                ...
                fvPatchScalarField& pmu = tmpMu.boundaryField()[patchi];
                fvPatchScalarField& pkappat = tmpKappat.boundaryField()[patchi];
                fvPatchScalarField& pkappar = tmpKappar.boundaryField()[patchi];
    ...
    {        
        ...
        fvPatchScalarField& pkappave = tmpKappave.boundaryField()[patchi];
        fvPatchScalarField& palphave = tmpAlphave.boundaryField()[patchi];

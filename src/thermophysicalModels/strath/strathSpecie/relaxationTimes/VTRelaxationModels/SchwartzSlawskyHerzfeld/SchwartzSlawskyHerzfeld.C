...
{   
    ...
    {    
        ...
        sigma12_ = dict2T.subDict("SchwartzSlawskyHerzfeldCoefficients").subDict("LennardJonesParameters").subDict(subDictName).lookup("sigma");
        epsilon12_ = dict2T.subDict("SchwartzSlawskyHerzfeldCoefficients").subDict("LennardJonesParameters").subDict(subDictName).lookup("epsilonBykB");
...
Foam::VTRelaxationModels::SchwartzSlawskyHerzfeld::tauVT() const
{
    ...
    {    
        ...
        forAll(this->Tt_.boundaryField(), patchi)
        {
            ...
            fvPatchScalarField& ptauVT = tauVT.boundaryField()[patchi];

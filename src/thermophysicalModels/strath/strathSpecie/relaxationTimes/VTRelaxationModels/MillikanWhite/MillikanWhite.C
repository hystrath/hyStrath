    ...
    )
{
    const scalar W1 =
        readScalar
        (
            dictThermoPhy.subDict(name1).subDict("specie")
                .lookup("molWeight")
        );
    const scalar W2 =
        readScalar
    ...
    const word MWCoeffDict = "MillikanWhiteCoefficients";
    ...
    if (not VTFullCoeffsForm_)
    {
        ...
        {
            if (dict2T.subDict(MWCoeffDict).isDict(name1+"_"+name2))
            {
                subDictName = name1+"_"+name2;
            }
            else if (dict2T.subDict(MWCoeffDict).isDict(name2+"_"+name1))
            {
                subDictName = name2+"_"+name1;
            }
            else if (dict2T.subDict(MWCoeffDict).isDict(name1))
            {
                subDictName = name1;
            }
            else
        ...
        (
            VTSpeciesDependent_ and dict2T.subDict(MWCoeffDict).isDict(name1)
        )
        {
            subDictName = name1;
        }
        ...
        preAij =
            readScalar
            (
                dict2T.subDict(MWCoeffDict).subDict(subDictName)
                    .lookup("preAij")
            );
        preMij =
            readScalar
    ...
    offset_ =
        readScalar
        (
            dict2T.subDict(MWCoeffDict).subDict(subDictName).lookup("offset")
        );
}

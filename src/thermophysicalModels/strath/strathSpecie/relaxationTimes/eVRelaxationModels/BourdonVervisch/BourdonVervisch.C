...
{   
    ...
    else 
    {
        ...
        BV1_ = dict2T.subDict("BourdonVervischCoefficients").subDict(subDictName).lookupOrDefault<FixedList<scalar,2> >("BV1", defaultList);
        BV2_ = dict2T.subDict("BourdonVervischCoefficients").subDict(subDictName).lookupOrDefault<FixedList<scalar,2> >("BV2", defaultList);
        BV3_ = dict2T.subDict("BourdonVervischCoefficients").subDict(subDictName).lookupOrDefault<FixedList<scalar,2> >("BV3", defaultList);
    }
}
...
Foam::eVRelaxationModels::BourdonVervisch::taueV() const
{
    ...
    forAll(this->Tv_.boundaryField(), patchi)
    {
        ...
        fvPatchScalarField& ptaueV = taueV.boundaryFieldRef()[patchi];
